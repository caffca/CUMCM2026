...
状态：`LEXICOGRAPHIC_PREFIX_PROVED_SHIFT_INCUMBENT`。第一层最少撤销数已由 CP-SAT 上下界闭合证明为 **6**；随后用独立 CNF + CaDiCaL 对次级阈值做不可行性证明，精炼方案 B 已闭合到 B 类调整数。最后的平移量只保留当前可行上界，不宣称完整字典序最优。
...
| `S10`（上述前缀固定） | — | 775 | 当前方案上界，未证明 |

`revocation_bound.json` 保存 CP-SAT 的 `objective=6`、`best_objective_bound=6` 证书；`proof/sat_b_le_3.json` 证明 `B≤3` 不可行，`proof/sat_adjust_le_125.json` 证明 `M≤125` 不可行，另两份 SAT 证据闭合 `M_A=16` 与 `M_B=34`。外部工作簿只提供具体动作见证，动作本身由独立验证复核；次级最优性来自阈值不可行性证据。
...
- `q2_proof_matrix.csv` 及 `proof/` 下的独立 SAT/CP-SAT 阈值证据；
