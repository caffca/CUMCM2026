#!/usr/bin/env python3
"""Assemble the verified Q2 incumbent and its conditional lexicographic stages."""
...
def main() -> None:
    ...
    stage_paths = {
        ...
        "minimum_adjusted_given_revocations": args.minimum_adjusted,
        "preserve_A_given_previous": args.preserve_a,
        "preserve_B_given_previous": args.preserve_b,
        "minimum_shift_given_previous": args.minimum_shift,
    }
    stages = {name: load(path) for name, path in stage_paths.items()}
    final = stages["minimum_shift_given_previous"]
    validation = load(args.validation)
    final_summary = final["summary"]
    counts = final_summary["counts_by_category"]
    objective_values = {
        "revocations": stages["minimum_revocations"]["optimization"]["objective_value"],
        "adjusted_plans": stages["minimum_adjusted_given_revocations"]["optimization"]["objective_value"],
        "modified_A": stages["preserve_A_given_previous"]["optimization"]["objective_value"],
        "modified_B": stages["preserve_B_given_previous"]["optimization"]["objective_value"],
        "modified_C": sum(counts[category]["调整数量"] + counts[category]["撤销数量"] for category in ["C"]),
        "total_absolute_shift": stages["minimum_shift_given_previous"]["optimization"]["objective_value"],
    }
    ...
    payload = {
        ...
        "objective_interpretation": "将题面‘尽量’层级解释为字典序目标；A/B/C 优先级在撤销数和调整数固定后通过最小化 A、B 被修改数量体现。",
        "time_domain": [0, 643],
        ...
        "claim_boundary": {
            ...
            "paper_wording": "当前可写为‘在限时 MILP 得到的 61 撤销 incumbent 条件下，后续字典序阶段条件最优；全局最小撤销数尚未证明’，不可写成全局最优。" if not first_stage_proved else "各阶段均已证明最优。",
        },
    }
