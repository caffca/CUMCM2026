...
def feature_vectors(candidates: list[Candidate]) -> dict[str, list[int]]:
    return {
        ...
        # Category-C complements are useful when the total R and the A/B
        # prefix are fixed.  They are included explicitly so independent SAT
        # encoders can propagate the implied C counts instead of relying on a
        # weak global difference.
        "revoke_C": [int(c.category == "C" and c.action == "revoke") for c in candidates],
        ...
        "adjust_C": [int(c.category == "C" and c.action in {"freq", "time"}) for c in candidates],
