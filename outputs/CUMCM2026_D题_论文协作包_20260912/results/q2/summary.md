...
- 冻结给 Q3 的 `Q2-T-incumbent-v1` 在条件前缀 `C=6, M=120` 下为：`P_A=16, C_A=1, P_B=37, C_B=5, P_C=73`，`S_sum=830`；其余位移层未完成严格证明。
- 另有一个独立的同前缀保护层探索 incumbent：`P_A=15, C_A=1, P_B=38, C_B=5, P_C=73`，原始位移 `Σ|df|=619`、`Σ|dt|=114`，故 `S_sum=847`。该方案不替换 Q3 冻结输入。
- Q3 应固定使用：`Q2-T-incumbent-v1.json`、`selected_T_cells_lexfixed.json`、`result2_T_cells_lexfixed.xlsx`。
- 条件保护层文件：`selected_T_cells_tiebreak120_300.json`、`solver_T_cells_tiebreak120_300.json`、`result2_T_cells_tiebreak120_300.xlsx`。
...
- `C*=6` 是当前可写入论文的严格结论；T 的 `M=120` 以及 `P_A=16`（冻结快照）或 `P_A=15`（保护层探索）均为阶段性 incumbent/条件 incumbent，必须在正文中同时报告上下界与求解状态。
